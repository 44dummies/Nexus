        ...
        <div className="glass-panel rounded-2xl p-6">
            ...
            <div className="grid grid-cols-3 gap-4">
                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 text-center">
                    ...
                    <p className="text-xl font-mono font-bold text-emerald-400">+${totalProfitToday.toFixed(2)}</p>
                </div>
                <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 text-center">
                    ...
                    <p className="text-xl font-mono font-bold text-red-400">-${totalLossToday.toFixed(2)}</p>
                </div>
                <div className={`border rounded-xl p-4 text-center ${netPnL >= 0 ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-red-500/10 border-red-500/20'}`}>
                    <Zap className={`w-5 h-5 mx-auto mb-2 ${netPnL >= 0 ? 'text-emerald-400' : 'text-red-400'}`} />
                    <p className="text-xs text-muted-foreground uppercase">Net</p>
                    <p className={`text-xl font-mono font-bold ${netPnL >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
